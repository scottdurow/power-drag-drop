// To test from command line use the following from the repo root directory
...
module.exports = ({ majorVersion, minorVersion, buildVersion, manifestsPaths, solutionPaths, beta }) => {
    ...
    // Use the format YYMMDD for minor version if minorVersion = -1
    if (minorVersion.toString() == "-1") {
        const today = new Date();
        minorVersion = today.toISOString().substring(2, 4) + today.toISOString().substring(5, 7) + today.toISOString().substring(8, 10);
    }
    ...
    // As above, this would be better if we used xml parsing to ensure the correct version element is updated
